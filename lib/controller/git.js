    ...
    this.printRelationshipList = function (project) {
        ...
        return function () {
            var UsecaseCommit = db.models.usecase_commit,
                tab_arr = [["Use case", "Strength", "Relevance", "#Commit", "Files", "LoC", "UC LoC"]];
            ...
            return new Promise(function (f) {
                UsecaseCommit.find({
                    ...
                    for (var i in uc) {
                        var rel = uc[i];
                        tab_arr.push([rel.usecase.name, rel.within_percent.toFixed(2), rel.commit_relevance_percent.toFixed(2), rel.commit_sha.substring(0,7), rel.loc_sum, rel.files_sum, rel.uc_loc_sum])
                    }
