    ...
    model = require('../model')(function (err, db) {
        ...
        process.on('message', function(obj) {
            ...
            var Git = require("nodegit"),
                ...
                base_path = obj.argv_base_path,
                argv_skip = obj.argv_skip,
                repo = null,
                project = obj.project,
                            ...
                            commit.getDiff().then(function (diffList) {
                                    ...
                                    return diff.patches().then(function (patches) {
                                        ...
                                        return Promise.all(patches.map(function (patch) {
                                            ...
                                            return ctrl.getBlob(repo, commit, patch.newFile().path()).then(function (blob) {
                                                ...
                                                if (!utils.startsWith(newestBlobPath, watch_path)) {
                                                    logger.log(colors.yellow("File outside of PATH: "+newestBlobPath));
                                                    return new Promise(function (f) {f()});
                                                }

                                                if (argv_skip && new RegExp(argv_skip).test(newestBlobPath)) {

                                                    logger.log(colors.yellow("Skipping " + newestBlobPath));
                                                    return new Promise(function (f) {f()});
                                                }
                                                            ...
                                                            if (fragments.length === 0) {
                                                                ...
                                                                for (var i in fragments) {
                                                                    ...
                                                                    if (diffs.tree.children && diffs.tree.children.length > 0 && diffs.stats.changesSum !== 0) {
                                                                        ...
                                                                        if (p !== fragment.path) {
                                                                            ...
                                                                            if (!(newestBlobPath in usecase_commit_within[fragment.usecase_id])){
                                                                                ...
                                                                                usecase_commit_within[fragment.usecase_id][newestBlobPath] = {
                                                                                    ...
                                                                                    outside:        diffs.stats.changesSum,
                                                                                    percent_within: 0,
                                                                                    within:         false,
                                                                                    path_same: false,
                                                                                };
                                                                            ...
                                                                            if (!(fragment.path in files_commit_within)){

                                                                                files_commit_within[fragment.path] = {
                                                                                    changes:        diffs.stats.changesSum,
                                                                                    unchanged:      diffs.stats.unchanged,
                                                                        ...
                                                                        var percent_within = 100-((100*c.stats.outsideOfTree)/(diffs.stats.changesSum)); // -1 means 1 is the file, which is matched implicitly
                                                                        ...
                                                                        usecase_commit_within[fragment.usecase_id][newestBlobPath].path_same = true;
                                                                        ...
                                                                        if (base_path) 
                                                                            var base_p = path.resolve(path.join(base_path, fragment.path));
                                                                        else 
                                                                            var base_p = newestBlobPath;

                                                                        var fragmentBlock = parser.block.parse(fs.readFileSync(base_p, "utf-8"), fragment.lang, newestBlobPath);
                                                                        ...
                                                                        //files_commit_within[fragment.path].path_same = true;

                                                                        /*if (files_commit_within[fragment.path].percent_within < percent_within) {*/

                                                                            //files_commit_within[fragment.path].percent_within = percent_within;
                                                                            //files_commit_within[fragment.path].outside = c.stats.outsideOfTree;
                                                                            //files_commit_within[fragment.path].are = c.are;
                                                                            //files_commit_within[fragment.path].outside_tree = c.tree;
                                                                        /*}*/
                                ...
                                .then(function () {
                                    ...
                                    */
                                    ...
                                    var same_files_fragments_gte_2_count = 0;
                                    commit_db.struct_change_sum = struct_change_sum;
                                    commit_db.struct_change_in_head_sum = struct_change_in_head_sum;
                                    commit_db.same_files_fragments_gte_2_count = same_files_fragments_gte_2_count;
                                    ...
                                    commit_db.save(function (err) {
                                        ...
                                        function saveWithin(uc, commit_db, within_percent, within_sum, same_files_fragments_count) {
                                            return new Promise(function (f) {
                                                getUcLoC(uc, function (uc_loc_sum) {
                                                    ...
                                                    commit_db.save(function (err) {
                                                        ...
                                                        UsecaseCommit.create({
                                                            ...
                                                            same_files_fragments_count: same_files_fragments_count,
                                        ...
                                        function saveWithinF(p, commit_db, within_percent, within_sum) {
                                            return new Promise(function (f) {
                                        ...
                                        if (Object.keys(usecase_commit_within).length > 0)
                                            for (var uc in usecase_commit_within) {
                                                ...
                                                for (var filepath in upi) {
                                                    ...
                                                    changeSum += parseInt(upi[filepath].changes);
                                                ...
                                                var same_files_fragments_count = 0;
                                                for (var i in upi) {
                                                    if (upi[i].path_same) same_files_fragments_count++;
                                                }

                                                var withinSum = changeSum-outsideSum;//-same_files_fragments_count;
                                                
                                                var withinPercent = (((100*withinSum)/(changeSum)));
                                                if (changeSum  === 0) {withinPercent = 0};
                                                 
                                                if (same_files_fragments_count >= 2) {
                                                    same_files_fragments_gte_2_count++;

                                                }
                                                withins.push(saveWithin(uc, commit_db, withinPercent, withinSum, same_files_fragments_count));

                                            }

                                        if (Object.keys(files_commit_within).length > 0)
                                            for (var p in files_commit_within) {
                                                var upi = files_commit_within[p];

                                                var changeSum = parseInt(upi.changes),
                                                    outsideSum = parseInt(upi.outside);

                                                var withinSum = changeSum-outsideSum;
                                                var withinPercent = (((100*withinSum)/(struct_change_sum)));
                                                if (changeSum  === 0) {withinPercent = 0};
                                                 
                                                withins.push(saveWithinF(p, commit_db, withinPercent, withinSum));
                                        ...
                                        Promise.all(withins).then(function () {
                                            ...
                                            if (files_sum===0) commit_db.status += "skipped - no files in PATH;"
