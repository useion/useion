...
/**
 * Example use
 *    ./useion -g ../opencart/  ../opencart/upload/catalog/ -l 10 --sl 200  --skip "(jquery)|(bootstrap)|(font-awesome)|(language/en-gb)" --base_path /home/dash/Projects/opencart-oo/catalog/  > ../opencart.log2.md
 */
module.exports = function(db) {
    ...
    this.run = function(project, fin) {
        ...
        var Git = require("nodegit"),
            ...
            visitedCommits = {},
            processedCommits = {},
            skip_commits = argv.sl ? parseInt(argv.sl) : 0,
            skipped_commits = 0,
            max = 4,
            cur = 0;
        ...
        function iterate(commit, parentId, limit) {
            return new Promise(function(fi) {
                ...
                if (skip_commits > skipped_commits) {
                    skipped_commits++;
                    //fi();
                    //return true;
                } else
                            ...
                            return Promise.all(iterateParents).then(function() {
                                ...
                                if (!nextid)
                                    ...
                                    return repo.getCommit(nextid).then(function(commit) {
                                        return iterate(commit, parentId, limit).then(function() {
                                            fi()
                                        });
                                    })
                            })
        ...
        return new Promise(function(f) {
            ...
            //.then(ctrl.printRelationshipCount3(project, 0))
            //[>.then(ctrl.printRelationshipCount3(project, 25))<]
            //[>.then(ctrl.printRelationshipCount3(project, 50))<]
            //.then(ctrl.printRelationshipCount3(project, 75))




            /*.then(ctrl.dups(project))*/
                //.then(ctrl.dupsBase(project))
            ...
            //[>.then(ctrl.printRelationshipCount2(project, 0))<]
            ////.then(ctrl.printRelationshipCount2(project, 25))
            ////.then(ctrl.printRelationshipCount2(project, 50))
            ////.then(ctrl.printRelationshipCount2(project, 75))
            ...
            .then(function () {
            logger.log("A modularization for changes: "+formatDate(new Date()));
            })

            .then(ctrl.findAndPrintModulesBasedOnChanges(project))

            .then(function () {
            logger.log("Finished: "+formatDate(new Date()));
            })
    ...
    this.dups = function(project) {
        ...
        return function() {
            ...
            var UsecaseCommit = db.models.usecase_commit,
                ...
                removeUca = function(uca) {
                    return new Promise(function(fi) {
                        if (uca.length === 0) return fi();
                        else
                            UsecaseCommit.find({
                                or: uca
                            }).remove(function(err) {
                                if (err) throw err;
                                console.log("removed", uca.length);
                                fi();
                            })
                    })
                },
            ...
            return new Promise(function(f) {
                ...
                function bulk(limit, offset) {
                    ...
                    return new Promise(function(fa) {
                        UsecaseCommit.find({
                            ...
                            offset: offset,
                            limit: limit
                        }, ["commit_sha", "Z", "within_percent", "Z"]).run(function(err, uc) {
                            if (err) throw err;
                            ...
                            for (var i in uc) {
                                ...
                                uca = uc[i];
                                if (uca.commit_sha != last) {
                                    // skip
                                    //console.log(uca.commit_sha, uca.within_percent);
                                } else {
                                    // remove
                                    //console.log("removing", uca.commit_sha, uca.within_percent);
                                    //remove.push(removeUca(uca))
                                    remove.push({
                                        id: uca.id
                                    })
                                }
                            ...
                            if (uc.length === 0)
                                ...
                                bulk(limit, offset + limit).then(function() {
                                    fa();
                                });

                            /*removeUca(remove).then(function() {*/
                ...
                function bulkRem (i) {
                    ...
                    return new Promise(function (fa) {
                        ...
                        removeUca(allRemove[i]).then(function () {
                            i++;
                            if (allRemove.length <= i)
                                fa();
                            else
                                bulkRem(i).then(function () {
                                    fa();
                                })
                        })
                ...
                bulk(20, 0).then(function() {
                    bulkRem(0).then(function () {
                        f();
                    });
                });
    ...
    this.dupsBase = function(project) {

        return function() {

            var BaseFileCommit = db.models.base_file_commit,
                last = null,
                ...
                function bulk(limit, offset) {
                    ...
                    return new Promise(function(fa) {
                        ...
                        BaseFileCommit.find({
                            ...
                            , ["commit_sha", "Z", "within_percent", "Z"]).only("id", "commit_sha", "within_percent", "path", "commit_project_id").run(function(err, uc) {
                            if (err) throw err;
...
/*                            removeUca(remove).then(function() {*/
    ...
    this.getGroupsFromCommits = function(project, groups) {
        ...
        return new Promise(function(f) {
            ...
            function getLoC(commits, cb) {
                ...
                StructChange.aggregate(["tree_path_lvl_id", "loc_without_children"], {
                    project_id: project.id,
                    or: commits,
                    in_head: true,
                    change_type: orm.not_in(["unchanged"])
                }).avg("loc_without_children").groupBy("tree_path_lvl_id").get(function(err, stats) {
                    if (err) throw err;
            ...
            function getTheMostSimilar(commits) {
                ...
                return new Promise(function(fi) {
                    StructChange.find({
                        ...
                        StructChange.aggregate(["commit_sha"], {
                            project_id: project.id,
                            or: q,
                            commit_sha: orm.not_in(n),
                            in_head: true,
                            change_type: orm.not_in(["unchanged"])
                        }).count().groupBy("commit_sha").order("-count").get(function(err, stats) {
                            if (err) throw err;
                            ...
                            if (stats.length === 0) {
                                ...
                                commits.push({
                                    commit_sha: mostSimilarCommitSha
                                });
                                ...
                                getLoC(commits, function(loc) {
                                        ...
                                        commits: commits,
                                        loc: loc,
                                        mostSimilarCommitSha: mostSimilarCommitSha
                                    });
                                });
            ...
            function whileLoC(commits, maxLoC) {
                return new Promise(function(fi) {
                    ...
                    logger.log("Finding similar commits for group "+formatDate(new Date())+utils.dumpObject(commits));
            ...
            return Promise.all(grpsPromises).then(function() {
                    ...
                    var maxCG = 500;
                logger.log("Selecting the best commit groups: "+formatDate(new Date()));
                return CommitGroup.aggregate(["id", "commit_count", "loc", "commit_shas_sorted"], {}).count().groupBy("commit_shas_sorted").order("-commit_count").limit(maxCG).get(function(err, commit_groups) {
                    if (err) throw err;

                    function getCommitGroupInfo(cg) {
                        return new Promise(function(f) {

                            logger.log("getting info for commit group "+cg.id+": "+formatDate(new Date()));

                            CommitGroupCommit.find({
                                commit_group_id: cg.id
                            }, function(err, cgc) {
                                var relevance_sum = 0,
                                    relevance_count = 0;
                                cg.commits = [];
                                ...
                                for (var i in cgc) {
                                    ...
                                    cgq.push({
                                        commit_sha: cgc[i].commit.sha
                                    })
                                }
                                cg.relevance = relevance_sum / relevance_count;


                                StructChange.aggregate(["tree_path_lvl1", "tree_path_lvl2", "tree_path_lvl3", "name", "change_type", "type"], {
                                    project_id: project.id,
                                    or: cgq,
                                    in_head: true,
                                    change_type: orm.not_in(["unchanged"])
                                }).count().groupBy("tree_path_lvl_id").get(function(err, stats) {
                                    if (err) throw err;
                                    ...
                                    for (var i in l2) {
                                        if (l2[i].l && l2[i].end)
                                            if (struct[l2[i].stats.tree_path_lvl1])
                                    ...
                                    for (var i in l3) {
                                        ...
                                        if (l3[i].l && l3[i].end)
                                            if (struct[l3[i].stats.tree_path_lvl1] && struct[l3[i].stats.tree_path_lvl1].children[l3[i].stats.tree_path_lvl2])
    ...
    this.printCommitsWithoutRelationship = function(project) {
        ...
        return function() {
            ...
            return new Promise(function(f) {
                ...
                UsecaseCommit.aggregate(["commit_sha", "commit_relevance_percent"], {
                    commit_project_id: project.id
                }).sum("within_percent").groupBy("commit_sha").get(function(err, stats) {
                    // console.log(stats);
    ...
    this.printRelationshipCount2 = function(project, relevance) {
            ...
            return new Promise(function(f) {
                ...
                Usecase.find({
                    ...
                    var getCount = function(usecase_name, usecase_id, i, strength, eq) {
                        return new Promise(function(fi) {
                            ...
                            var criteria = {
                                ...
                                commit_project_id: project.id,
                                same_files_fragments_count: orm.gt(strength),
                                commit_relevance_percent: orm.gt(relevance)
                            }
                            if (eq) {
                                criteria["same_files_fragments_count"] = strength;
                            }
                    ...
                    for (var i in usecases) {
                        ...
                        for (var j = 0; j <= 9; j++) {
                            prom_ucs.push(getCount(usecase.name, usecase.id, j, j, true))
                        }
                        prom_ucs.push(getCount(usecase.name, usecase.id, 10, 9, false))
                    }
                    ...
                    Promise.all(prom_ucs).then(function() {
                        ...
                        logger.log("");
                        logger.logH1("Relationship count with file strength > X and relevance > " + relevance + "");
                        logger.log("");
                        logger.log(table(tab_arr));
                        // logger.log("");
    ...
    this.printRelationshipCount3 = function(project, relevance) {

        return function() {
            var Fragment = db.models.fragment,
                BaseFileCommit = db.models.base_file_commit,
                ucs = {};
            ...
            return new Promise(function(f) {
                ...
                Fragment.find({
                    ...
                    if (err) throw err;
                    var getCount = function(fragment_path, i, strength1, strength2, eq) {
                        return new Promise(function(fi) {

                            var criteria = {
                                path: fragment_path,
                                commit_project_id: project.id,
                                within_percent: orm.gt(strength1),
                                and: [{
                                    within_percent: orm.lte(strength2)
                                }, ],
                                commit_relevance_percent: orm.gt(relevance)
                            }
                            if (eq) {
                                criteria["within_percent"] = strength1;
                                delete criteria["and"];
                            }

                            BaseFileCommit.count(criteria, function(err, count) {
                                if (err) throw err;
                                if (!(fragment_path in ucs)) ucs[fragment_path] = {};
                                ucs[fragment_path][i] = {
                    ...
                    for (var i in fragments) {
                        ...
                        prom_ucs.push(getCount(fragment.path, -1, 0, 0, true))
                        for (var j = 0; j <= 9; j++) {
                            prom_ucs.push(getCount(fragment.path, j, j * 10, (j * 10) + 10, false))
                        }
                        prom_ucs.push(getCount(fragment.path, 10, 100, 0, true))
                    }
                    ...
                    Promise.all(prom_ucs).then(function() {
                        ...
                        var tab_arr = [],
                            head = [];
                        head.push("Fragment path");
                        head.push("=0");
                        for (var j = 0; j <= 9; j++) {
                            head.push("+10>" + (j * 10)); //+";"+"R>"+(j*20));
                        }
                        head.push("=100");
                        tab_arr.push(head);
                        ...
                        logger.log("");
                        logger.logH1("Relationship count (base code) with block strength > X and relevance > " + relevance + "");
                        logger.log("");
                        logger.log(table(tab_arr));
                        // logger.log("");
                        // logger.log("Legend: strength; relevance");
        ...
        return function() {
            ...
            return new Promise(function(f) {
                ...
                Usecase.find({
                    ...
                    if (err) throw err;
                    var getCount = function(usecase_name, usecase_id, i, strength1, strength2, eq) {
                        return new Promise(function(fi) {
                    ...
                    for (var i in usecases) {
                        ...
                        prom_ucs.push(getCount(usecase.name, usecase.id, -1, 0, 0, true))
                        for (var j = 0; j <= 9; j++) {
                            prom_ucs.push(getCount(usecase.name, usecase.id, j, j * 10, (j * 10) + 10, false))
                        }
                        prom_ucs.push(getCount(usecase.name, usecase.id, 10, 100, 0, true))
                    }
                    ...
                    Promise.all(prom_ucs).then(function() {
                        ...
                        logger.log("");
                        logger.logH1("Relationship count with block strength > X and relevance > " + relevance + "");
                        logger.log("");
                        logger.log(table(tab_arr));
                        // logger.log("");
                        // logger.log("Legend: strength; relevance");
    ...
    this.printRelationshipList = function(project) {
        ...
        return function() {
            var UsecaseCommit = db.models.usecase_commit,
                tab_arr = [
                    ["Use case", "#Commit", "Block Strength [percent]", "File Strength [count]", "Relevance", "Files", "LoC", "UC LoC"]
                ];
            ...
            return new Promise(function(f) {
                UsecaseCommit.find({
                    ...
                    for (var i in uc) {
                        var rel = uc[i];
                        tab_arr.push([rel.usecase.name, rel.commit_sha.substring(0, 7), rel.within_percent.toFixed(2), rel.same_files_fragments_count, rel.commit_relevance_percent.toFixed(2), rel.files_sum, rel.loc_sum, rel.uc_loc_sum])
                    }
            ...
            db.driver.execQuery("delete from commit_group_commit;", function(err, data) {
                ...
                db.driver.execQuery("delete from commit_group;", function(err, data) {
                    ...
                    db.driver.execQuery("delete from usecase_commit;", function(err, data) {
                        ...
                        db.driver.execQuery("delete from base_file_commit;", function(err, data) {
                            if (err) throw err;
