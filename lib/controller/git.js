    ...
    this.printRelationshipCount3 = function (project, relevance) {
        ...
        return function () {
            ...
            return new Promise(function (f) {
                ...
                Fragment.find({project_id: project.id}, function (err, fragments) {
                    if (err) throw err;
                    var getCount = function (fragment_path, i, strength, eq) {
                        return new Promise(function (fi) {
                    ...
                    for (var i in fragments) {
                        ...
                        prom_ucs.push(getCount(fragment.path, -1, 0, true))
                        for (var j=0; j<=9; j++) {
                            prom_ucs.push(getCount(fragment.path, j, j*10, false))
                        }
                        prom_ucs.push(getCount(fragment.path, 10, 100, true))
                    }
            ...
            return new Promise(function (f) {
                ...
                Usecase.find({project_id: project.id}, function (err, usecases) {
                    ...
                    for (var i in usecases) {
                        ...
                        for (var j=0; j<=9; j++) {
                            prom_ucs.push(getCount(usecase.name, usecase.id, j, j*10, false))
                        }
                        prom_ucs.push(getCount(usecase.name, usecase.id, 10, 100, true))
                    }
    ...
    this.printRelationshipList = function (project) {
        ...
        return function () {
            var UsecaseCommit = db.models.usecase_commit,
                tab_arr = [["Use case", "#Commit", "Block Strength [percent]", "File Strength [count]", "Relevance", "Files", "LoC", "UC LoC"]];
